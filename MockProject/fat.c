...
uint8_t read_boot_sector(data_boot_sector *a)
{
	...
	a->bytePerSector= little(data[11], data[12]);
    a->clusterPerSector= data[13];
    a->sectorPerFat12= little(data[22],data[23]);
    a->numSector= little(data[19], data[20]);
    a->sectorBootDirector= little(data[17], data[18]);
    for(i=0; i<8; i++)
    {
...
uint8_t read_root_directory(data_entry *a, uint8_t *num_entry, uint32_t cluster,uint16_t num_sec)
{
	*num_entry = 0;
	...
	uint8_t j=0;
	uint8_t k=0;
	for(i=0; i<512*num_sec && data[j*32] != 0; i++)
	{
		j= i/32;
		if(data[j*32+11] != 15 && data[j*32] != 0 &&data[j*32+11] != 0x28 )//&data[j*32+11]!=16.&& data[j*32] != 46
		{
			...
			{
				(a + (k/32))->cluster = little(data[i], data[i+1]);
			}
			k++;
		}
	}
